...
trait ExcelXmlHelpers
{
        ...
        if ($currentIdx !== false && $currentIdx < $periods->count() - 1) {
            ...
            if ($prevReport) {
                ...
                $prevResults = Result::where('reportable_type', get_class($prevReport))
                    ...
                    ->get()->keyBy('item_id');
                $prevCategories = $this->getAllCategories();
                $score = 0;
                foreach ($prevCategories as $cat) {
                    foreach ($cat->items as $item) {
                        $r = $prevResults->get($item->id);
                        $score += \App\Services\ScoreCalculator::calculateItemScore($item, $r);
                    }
                }
                return $score;
            }
    ...
    {
        $report = $this->modelClass()::withoutGlobalScope('no_pairing')->with('gerai', 'user', 'results.item.criteria', 'results.criterion')->findOrFail($id);
    ...
    protected function buildExcel($report, array $headerReplacements, $outputDir = null)
    {
        ...
        $results = $report->relationLoaded('results')
            ? $report->results->keyBy('item_id')
        ...
        foreach ($categories as $cat) {
            foreach ($cat->items as $item) {
                ...
                $score = ScoreCalculator::calculateItemScore($item, $r);
                $totalScore += $score;
                $items[$item->name] = [
                    'category' => $cat->name,
        ...
        foreach ($periodReports as $pr) {
            ...
            foreach ($pr->results as $result) {
                ...
                $item = $result->item;
                $score = ScoreCalculator::calculateItemScore($item, $result);
        ...
        $penjelasanItemsAll = Item::with('criteria')
            ->whereIn('id', collect($penjelasanGroups)->pluck('item_ids')->flatten())
            ->get()
            ->keyBy('id');
        foreach ($penjelasanGroups as $group) {
            ...
            foreach ($group['item_ids'] as $itemId) {
                ...
                if (!$gr) continue;
                $gItem = $penjelasanItemsAll->get($itemId);
        ...
        $headerReplacements = array_merge($headerReplacements, [
            ...
            '{minor}'            => $report->minor ?? '-',
            '{mayor}'            => $report->major ?? '-',
            '{peringatan_awal}'  => $report->peringatan_awal ?? '-',
        ]);
        ...
        $typeDisplay = match($this->type) {
            'monitoring' => 'Monitoring',
            'pra-monitoring' => 'Pramonitoring',
            're-monitoring' => 'Remonitoring',
            default => ucfirst($this->type),
        };
        $periode = $report->periode_label ?? $report->checkin_at?->setTimezone($tz)->locale('id')->isoFormat('MMMM YYYY') ?? $report->tanggal?->locale('id')->isoFormat('MMMM YYYY') ?? '';
        $filename = "{$typeDisplay} - {$report->gerai->kode_gerai} - {$periode}.xlsx";
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if ($name !== 'xl/sharedStrings.xml' && !str_contains($content, '{')) continue;
        ...
        // Build shared string map once after Phase 2 (reused by later sections)
        $cachedSsContent = $zip->getFromName('xl/sharedStrings.xml');
        $cachedSsMap = [];
        if ($cachedSsContent !== false) {
            $cachedSsDom = new DOMDocument;
            $cachedSsDom->loadXML($cachedSsContent);
            foreach ($cachedSsDom->getElementsByTagName('si') as $idx => $si) {
                $t = $si->getElementsByTagName('t')->item(0);
                $cachedSsMap[$idx] = $t ? $t->textContent : '';
            }
        }
        ...
        if ($tmpZip->open($templatePath) === true) {
            ...
            if ($tmpSS && $tmpS3 && $tmpS2) {
                ...
                if ($sheet2Content !== false) {
                    ...
                    $penjelasanIsi = ($this->hasPenjelasanFormulir2()) ? ($report->penjelasan_isi ?? []) : [];

                    if ($this->fillSheet2Custom($dom2, $xpath2, $ns2, $findingLines, $report, $lowItems, $sheet3ZeroItems, $items, $zip)) {
                        // Custom handler (pra-monitoring) took care of Sheet2

                    } elseif ($isNonTemuan) {
                        // === Non Temuan: template unchanged, paste below PENJELASAN ===
                        $ssTextMap = $cachedSsMap;
                ...
                if ($sheet3Content !== false) {
                    ...
                    if ($paRow && $noteRow) {
                        ...
                        $infoRows[] = static::xmlMakeBRow($dom3, $ns3, 'Kondisi cat: ' . ($report->kondisi_cat ?: 'Baik'), $infoRn);
                        $infoRows[] = static::xmlMakeBRow($dom3, $ns3, 'Kondisi awning: ' . ($report->kondisi_awning ?: 'Baik'), $infoRn);
                        $infoRows[] = static::xmlMakeBRow($dom3, $ns3, 'Kondisi vinyl reklame dinding/jalan: ' . ($report->kondisi_vinyl ?: 'Baik'), $infoRn);
                        $infoRows[] = static::xmlMakeBRow($dom3, $ns3, 'Kondisi stiker kaca: ' . ($report->kondisi_stiker_kaca ?: 'Baik'), $infoRn);
                        $infoRows[] = static::xmlMakeBRow($dom3, $ns3, '', $infoRn);
        ...
        if ($sheet4Content !== false) {
            ...
            foreach ($filledCols as $col) {
                ...
                $cell->removeAttribute('t');
                $v = $dom4->createElementNS($ns4, 'v');
                $v->textContent = (string) MonitoringReport::GRADE_B_THRESHOLD;
                $cell->appendChild($v);
            }
